classdef Define_LoadPathGUIApp < handle
    %
    % Usage:
    %     app.LoadPathGUIApp
    ...
    properties (Constant)
        AppPathBase = {'/LoadPathGUI/code'};
        AppClass = 'LoadPathGUIApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            LoadPathGUIApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'LoadPathGUI')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong LoadPathGUI
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'LoadPathGUI').getString, ...
            ...
            if nargout(@LoadPathGUI) == 0
                eval('LoadPathGUI');
            else
                obj.AppHandle = eval('LoadPathGUI');
            end
